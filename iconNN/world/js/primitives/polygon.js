class Polygon {
    constructor(points) {
        ...
        for(let i = 1; i <= points.length; i++) {
            this.segments.push(
                // loop around and last points.length -> 0 index
    ...
    static load(info) {
        return new Polygon (
            info.points.map((e)=> new Point(e.x, e.y))
        );
    }
        ...
        for (let i = 0; i < segs1.length; i++) {
            for(let j = 0; j < segs2.length; j++) {
                ...
                if(int && int.offset != 1 && int.offset != 0) {
                    ...
                    let aux = segs1[i].p2;
                    segs1[i].p2 = point;
                    segs1.splice(i + 1, 0, new Segment(aux, point));
                    aux = segs2[j].p2;
                    segs2[j].p2 = point;
                    segs2.splice(j + 1, 0, new Segment(aux, point));
                }
            }
        }
    ...
    containsPoint(point) {
        // if inside polygon intersection amount is odd
        // if outside polygon intersection amount is even
